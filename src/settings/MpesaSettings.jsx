...
//   return (
//     <PaymentGatewayOtpGate title="M-Pesa Settings">
//     <>
...
// ---- SasaPay (frontend only for now, no backend wiring yet) ----
const [sasaPaySettings, setSasaPaySettings] = useState({
  client_id: '',
  client_secret: '',
  merchant_code: '',
});
...
const handleChangeSasaPaySettings = (e) => {
  const { name, value } = e.target;
  setSasaPaySettings((prevData) => ({
    ...prevData,
    [name]: value,
  }));
};

const saveSasaPaySettings = (e) => {
  e.preventDefault();
  // NOTE: frontend only for now — backend/API wiring to follow.
...
<TextField
  ...
  <TextField
      ...
      {/* SasaPay Section (frontend only for now — backend integration to follow) */}
      <motion.div
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
      ...
      >
        <div className='flex justify-between'>
          <p className="text-xl font-semibold mb-4 flex items-center">
            <Payment className="mr-2" /> SasaPay
          </p>
        </div>

        <form onSubmit={saveSasaPaySettings}>
          <Box
            component="div"
            sx={{
              "& .MuiTextField-root": { m: 1, width: "100%", marginTop: 5 },
          ...
          >
            <TextField
              onChange={handleChangeSasaPaySettings}
              name="client_id"
              value={sasaPaySettings.client_id}
              className='myTextField'
              label="Client ID"
              InputProps={{
                startAdornment: <VpnKey className="mr-2" />,
            ...
            <TextField
              onChange={handleChangeSasaPaySettings}
              name="client_secret"
              value={sasaPaySettings.client_secret}
              className='myTextField'
              label="Client Secret"
              InputProps={{
                startAdornment: <Lock className="mr-2" />,
            ...
            <TextField
              onChange={handleChangeSasaPaySettings}
              name="merchant_code"
              value={sasaPaySettings.merchant_code}
              className='myTextField'
              label="Merchant Code"
              InputProps={{
                startAdornment: <Business className="mr-2" />,
