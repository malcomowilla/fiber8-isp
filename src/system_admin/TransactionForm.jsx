...
import { Save, Server, Wifi, Building } from 'lucide-react';
...
export default function TransactionForm({ company, onSubmit }) {
  ...
  const handleQuickRecord = (type, category, desc, options = {}) => {
    setTransactionType(type);
    setCategory(category);
    setDescription(desc);
    if (options.intercompany) {
      setIsIntercompany(true);
      setCounterparty(options.counterparty);
    }
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {(selectedCompany === 'saas' || selectedCompany === 'isp') && (
