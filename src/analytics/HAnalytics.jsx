...
const getThisMonthRevenue = useCallback(
  async() => {
...
const getThisYearRevenue = useCallback(
  async() => {

    try {
    const response = await fetch('/api/this_year_revenue', {
      headers: { 'Content-Type': 'application/json', 'X-Subdomain': subdomain }
    });
    const data = await response.json();
    
    if (response.ok) {
      setThisYearRevenue(data)
    }
  } catch (error) {
    console.log(error)
  }
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Hotspot Revenue Dashboard</h1>
          <p className="text-gray-600 dark:text-white">MPESA revenue tracking</p>
        </div>
        <button 
          onClick={() => {
            fetchRevenueSummary();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Daily Traffi`c Chart */}
        <div className="bg-white rounded-xl shadow-lg p-6 lg:col-span-2">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-gray-900">Hourly Transactions</h2>
            <input 
              type="date" 
              value={selectedDate}
          ...
          <ReactApexChart 
            options={dailyTrafficChart.options} 
            series={dailyTrafficChart.series} 
            type="bar" 
            height={350} 
          />
          <div className="mt-4 text-center">
            <button 
              onClick={() => fetchDailyRevenue(selectedDate)}
              className="px-6 py-3 bg-blue-500 text-white rounded-lg
               hover:bg-blue-600 transition-colors flex items-center gap-3"
            >
              Load Data for Selected Date
              <RefreshCw size={18} className={`mr-2 ${loadingDailyRevenue ? 'animate-spin' : ''}`} />
            </button>
          </div>
        </div>
