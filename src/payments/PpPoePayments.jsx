...
import PpPoePaymentDetails from './PpPoePaymentDetails';
...
const recordPayment = async (e) => {
  ...
  try {
    const response = await fetch('/api/pp_poe_mpesa_revenues', {
      ...
      body: JSON.stringify({
        customer: recordPaymentForm.user,
...
const fetchSubscribers = useCallback(
  ...
  try {
    setLoading(true);
    const response = await fetch(`/api/pp_poe_mpesa_revenues/${id}`, {
      method: 'DELETE',
      headers: {
        'X-Subdomain': subdomain,
        'Content-Type': 'application/json',
    ...
    },
    {
      ...
      render: (rowData) => (
        <div className="flex items-center gap-2">
          ...
          <span>{rowData.customer_name || 'Anonymous'}</span>
        </div>
      ),
    },
    {
...
<RecordPpoePaymentDetails openRecordDetails={openRecordDetails}
 setOpenRecordDetails={setOpenRecordDetails} 
 payment={selectedPayment} customers={customers}
 setRecordPaymentForm={setRecordPaymentForm}
            ...
            title={
              <div className="flex items-center gap-3 p-4">
                <div className="p-2  bg-green-500
                 rounded-lg">
                  <GoPeople className="w-6 h-6 text-white" />
                </div>
