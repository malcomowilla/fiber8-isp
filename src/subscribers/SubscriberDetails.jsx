...
e.preventDefault()
    try {
      ...
      const url = formDataSubscriber.id ? `/api/update_subscriber/${formDataSubscriber.id}` : '/api/subscriber';
      const method = formDataSubscriber.id ? 'PATCH' : 'POST';
...
"Content-Type" : "application/json",
        ...
        body: JSON.stringify({...formDataSubscriber, 
...
if (formDataSubscriber.id) {
        navigate('/admin/pppoe-subscribers')

  setTableDataSubscriber(tableDataSubscriber.map(item => item.id === newData.id ? newData : item))
  toast.success('Subscriber updated successfully', {
    ...
    navigate('/admin/pppoe-subscribers')
  setTableDataSubscriber([...tableDataSubscriber, newData])
...
<TextField 
            ...
            className='myTextField' fullWidth
                ...
                value={formDataSubscriber.building_name}
          ...
          <div className="">
            ...
            >
              ...
              <TextField
                className='myTextField'
                id="phone_number"
                value={formDataSubscriber.phone_number}
...
{/*             
          ...
          <div>
            ...
            <TextField 
            onChange={handleChangeBuildingNameAndHouseNumber}
            value={formDataSubscriber.house_number}
            name="house_number"
            InputProps={{
...
<p className=''> if you have chosen pppoe username as an 
  ...
  value={nodes.find(n => n.name === formDataSubscriber.node) || null}
...
/>
              ...
              {/* {position && ( */}
                ...
                startAdornment: <TbWorldLatitude className="mr-2" />,
                    ...
                    value={formDataSubscriber.latitude}
                  />
                ...
                startAdornment: <TbWorldLongitude className="mr-2" />,
                  ...
                  onChange={handleChangeSubscriber}
                  value={formDataSubscriber.longitude}
                    label="Longitude"
          ...
          text-lg' type="submit">
              {editingSubscriber ?   loading ?  'Updating...' : 'Update' : loading ? 'Saving...' : 'Save'}
            </button>
          </div>
        </form>
      </motion.div>
