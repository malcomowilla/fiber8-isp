...
setSubscriberSettings(
  {...subscriberSettings, prefix:prefix, minimum_digits:minimum_digits, 
   ...
   lock_account_to_mac: lock_account_to_mac
  }
)
      }else{
...
const handleUnBlockService = async (subscriptionData) => {
  setUnblockLoading(true);
  setUnblocking(true)
  try {
    const response = await fetch('/api/unbock_service', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Subdomain': subdomain
    ...
    if (response.ok) {
      ...
      toast.error(result.error || 'Failed to unblock service', {
        position: 'top-center',
        duration: 3000
      });
    ...
    setUnblocking(false);
     ...
     setUnblockLoading(false);
    setUnblocking(false);
    toast.error('Network error while unblocking service', {
      position: 'top-center',
      duration: 3000
    });
    console.error('UnBlock service error:', error);
  } finally {
    setUnblockLoading(false);
    setUnblocking(false);
  }
...
useEffect(() => {
    const subscription = cable.subscriptions.create(
      { channel: "OnlineStatsChannel" }, // must match your Rails channel class
      {
        received(data) {
            ...
            render: rowData => {
              ...
              const statusConfig = {
                ...
                blocked_online: {
                  ...
                  color: 'text-red-700',
                  bg: 'bg-red-50',
                  tooltip: 'Device is currently blocked and is currently online',
                  expiry: statusInfo.expiry
                },


                blocked_offline_never_connected: {
                  icon: (
                    <div className="relative inline-flex items-center">
                      <MdOutlineBlock className="text-red-700" />
                    </div>
                  ),
                  text: 'Blocked-Never Connected',
                  color: 'text-red-700',
                  bg: 'bg-red-50',  
                  expiry: statusInfo.expiry

                },
                offline_never_connected: {
                  icon: (
                    <div className="relative inline-flex items-center">
                      <MdOutlineBlock className="text-green-700" />
                    </div>
                  ),
                ...
                error: {
                 ...
                 blocked_offline: {
                  ...
                  text: `${statusInfo.last_seen} - Blocked`,
                  // last_seen: statusInfo.last_seen ? `Last seen: ${statusInfo.last_seen}` : 'Offline',
                  color: 'text-red-700',
                  bg: 'bg-red-50',
                  tooltip: 'Device is currently blocked and is currently offline',
                  expiry: statusInfo.expiry
                },
              };
          
              // First check if status is blocked, then fall back to other statuses
              const currentStatus = statusInfo.status === 'blocked_offline' ? 'blocked_offline' : statusInfo.status 
                // ? 'blocked-offline' 
                // : statusInfo.status in statusConfig 
                //   ? statusInfo.status === 'offline' ? 'offline' : 'blocked_online'
                //   : 'offline';
          
              const { icon, text, color, bg, tooltip, expiry } = statusConfig[currentStatus];
...
<div className='p-4 flex bg-yellow-50 rounded-lg shadow-sm'>
   ...
   <span className='flex text-lg text-orange-400'>Note- <p className=''>Editing subscription 
    details will affect the current connected session.</p> </span>
    </div>
  ): null}
...
{service_type === 'pppoe' ?  (
          ...
          <motion.div whileHover={{ scale: 1.01 }}>
            <TextField
              ...
              onChange={(e) => {
                ...
                setPppoePassword(e.target.value)
              }}
          ...
          <>
            ...
            <motion.div whileHover={{ scale: 1.01 }}>
              <TextField
                ...
                // id="ppoe_username"
                onChange={(e) => {
                   setPpoeUsername(e.target.value)
                }}
  ...
  className='myTextField'
    options={ips}
    value={formDataSubscriber.ip_address || null}
    onChange={(event, newValue) => {
      handleIpChange({ 
        target: { 
          name: "ip_address", 
            ...
            >
              <Autocomplete
               onChange={(event, newValue) => {
                ...
                value={packageName.find((pkg) => pkg.name === package_name) || null}
                options={packageName}
                getOptionLabel={(option) =>  {
                  return option.name
                }}   
                renderInput={(params) => (
                  <TextField className='myTextField' {...params} label="Select Package" 
...
{editingSubscription ? (
  ...
  <Tooltip title="Refresh service">
    <GiCycle className='text-green-500 cursor-pointer hover:text-green-700 w-6 h-6'/>
              <p>Refresh</p>
      ...
      {blockLoading && blocking ? (
        ...
        <MdOutlineBlock 
          className={`cursor-pointer text-2xl ${
            blockedSubscriptions.includes(formDataSubscriber.id) 
              ? 'text-red-700 animate-pulse' 
              : 'text-red-500 hover:text-red-700'
          }`}
          onClick={() => handleBlockService(formDataSubscriber)}
        />
      )}
    </div>
    <p>Block Service</p>
    ...
    <div className="relative">
      {unblockLoading && unblocking ? (
        ...
        <MdOutlineBlock 
          ...
          onClick={() => handleUnBlockService(formDataSubscriber)}
        />
      )}
    </div>
    <p>UnBlock Service</p>
